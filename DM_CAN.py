...
class MotorControl:
    ...
    def recv(self):
        ...
        for packet in packets:
            ...
            CMD = packet[1]
            processed_id = self.__process_packet(data, CANID, CMD)
            if processed_id:
    ...
    def __process_packet(self, data, CANID, CMD):
        if CMD == 0x11:
            motor_id_to_process = None
            if CANID != 0x00:
                if CANID in self.motors_map:
                    motor_id_to_process = CANID
            else:
                MasterID = data[0] & 0x0f
                if MasterID in self.motors_map:
                    motor_id_to_process = MasterID
            
            if motor_id_to_process:
                motor = self.motors_map[motor_id_to_process]
                ...
                # Return the MasterID associated with the motor object
                return motor.MasterID
        return None
